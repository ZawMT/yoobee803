...
def preprocess(df):
    ...
    for col, (lo, hi) in range_rules.items():
        ...
        if not bad.empty:
            print(f"  '{col}': {len(bad)} out-of-range values (expected {lo}–{hi}) — rows dropped")
    ...
    if not invalid_churned.empty:
        print(f"  'Churned': {len(invalid_churned)} invalid values — rows dropped")
...
def find_optimal_k(X_scaled, max_k=10):
    ...
    # Pick k at the largest drop in inertia
    drops = [inertias[i] - inertias[i + 1] for i in range(len(inertias) - 1)]
    best_k = list(k_range)[drops.index(max(drops))]
    return best_k
...
def cluster(df, n_clusters):
    ...
    for c in sorted(df["Cluster"].unique()):
        ...
        plt.scatter(subset["Steps_per_Day"], subset["Avg_Session_Duration_Min"],
                    label=f"Cluster {c}", alpha=0.6, edgecolors="k", linewidths=0.4)
